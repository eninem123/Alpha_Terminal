...
async function buildVipMarketSnapshot() {
    ...
    if (buf) {
      ...
      if (parsed && parsed.length) {
        for (const idx of parsed) {
          ...
          const amtStr = formatAmtBrief(idx.currentPrice * 1e6); // 粗估成交额
          indexLines.push(
            "- " + idx.name + "(" + idx.symbol + ") " +
            "最新 " + idx.currentPrice.toFixed(2) + " " +
            "涨跌 " + pctStr
          );
        }
      }
  ...
  if (indexLines.length) {
    lines.push("一、主要指数与宽基 ETF（点位、涨跌幅；数据来源：腾讯财经）");
    lines.push(...indexLines);
  } else {
    lines.push("一、指数与宽基 ETF：腾讯财经接口未返回数据（可能处于非交易时段）。");
  }
  ...
  try {
    const raw = await fetchTextNoStore(
      "https://vip.stock.finance.sina.com.cn/quotes_service/api/json_v2.php/Market_Center.getHQNodeDataSimple?page=1&num=12&sort=changepercent&asc=0&node=hs_a&symbol=",
      VIP_SNAPSHOT_FETCH_MS
    );
    if (raw) {
      const j = JSON.parse(raw);
      if (Array.isArray(j) && j.length) {
        for (let i = 0; i < Math.min(10, j.length); i++) {
          const r = j[i];
          const name = String(r.name || "").trim();
          const pct = Number(r.changepercent);
          const pctStr = (pct > 0 ? "+" : "") + pct.toFixed(2) + "%";
          sectorLines.push("- " + name + " 涨幅 " + pctStr);
        }
      }
    }
  } catch {}
  ...
  if (!sectorLines.length) {
    try {
      const buf2 = await fetchWithBuffer(
        "https://qt.gtimg.cn/q=s_sh000001,s_sz399001,s_sz399006,s_sh000300",
        VIP_SNAPSHOT_FETCH_MS
      );
      if (buf2) {
        const t2 = decodeTencentBody(buf2);
        const p2 = parseTencentIndexText(t2);
        for (const idx of p2) {
          const pct = idx.changePct;
          const pctStr = (pct > 0 ? "+" : "") + pct.toFixed(2) + "%";
          sectorLines.push("- " + idx.name + " 涨跌 " + pctStr);
        }
      }
    } catch {}
  }
  ...
  if (sectorLines.length) {
    lines.push("二、盘中强势股/板块热度（新浪涨幅榜前 10，仅供参考）");
    lines.push(...sectorLines);
  } else {
    lines.push("二、板块热度：未取到数据。");
  }
  ...
  if (newsItems.length) {
    lines.push("三、财经快讯标题（东财/新浪；含政策/产业关键词优先；全文以媒体原文为准）");
    for (const t of newsItems) lines.push("- " + t);
  } else {
    lines.push("三、财经快讯：本帧未能拉取。");
  }

  lines.push(
    "四、盘中核对要点（框架，非结论）：① 开盘后尽快浏览是否有科技/新能源/消费等产业政策或宏观数据发布，往往影响全天主线；② 开盘后约 30 分钟重点观察沪深300、中证500、科创50 及宽基 ETF 的成交额是否异常放大或萎缩；③ 融资余额请以上交所/深交所官方融资融券披露或专业终端为准。");
  return lines.join("\n");
}
