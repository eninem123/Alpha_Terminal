...
```powershell
$dir = Join-Path $HOME ".config\\ima"
New-Item -ItemType Directory -Force -Path $dir | Out-Null
Set-Content -Path (Join-Path $dir "client_id") -Value "your_client_id" -NoNewline
Set-Content -Path (Join-Path $dir "api_key") -Value "your_api_key" -NoNewline
```
...
Windows PowerShell 下建议使用 `curl.exe`（避免 PowerShell 对 `curl` 的别名行为）：

```powershell
function Ima-Api([string]$Path, [string]$Body) {
  curl.exe -s -X POST "https://ima.qq.com/$Path" `
    -H "ima-openapi-clientid: $env:IMA_OPENAPI_CLIENTID" `
    -H "ima-openapi-apikey: $env:IMA_OPENAPI_APIKEY" `
    -H "Content-Type: application/json" `
    -d $Body
}
```
